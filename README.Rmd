...
arrange(cleaned_data_hpi, hpi.6) %>% tail() %>% slice(1) %>% select(plant, fungus)
```
...
```{r}
arrange(cleaned_data_hpi, hpi.18) %>% tail() %>% slice(1) %>% select(plant, fungus)
```
...
#I already did this to make the clean_data_hpi
```
...
expr_data %>% group_by(gene.name, plant, fungus) %>% summarise(mean_expr = mean(expr.diff), var_exp = var(expr.diff)) %>% ungroup(gene.name) %>% arrange(mean_expr, var_exp) %>% group_by(plant, fungus) %>% tail %>% slice(1:3) 

expr_data %>% group_by(gene.name, plant, fungus) %>% summarise(mean_expr = mean(expr.diff), var_exp = var(expr.diff)) %>% ungroup(gene.name) %>% arrange(mean_expr, var_exp) %>% group_by(plant, fungus) %>% tail %>% slice(1:3) -> con_diff_data
